...
( function() {
	AmCharts[ "export" ] = function( chart, config ) {
		...
		var _this = {
			 ...
			 */
			gatherAttribute: function( elm, attr, limit, tov ) {
				var value, tov = tov ? tov : 0,
					limit = limit ? limit : 3;
				if ( elm ) {
					value = elm.getAttribute( attr );

					if ( !value && tov < limit ) {
						return _this.gatherAttribute( elm.parentNode, attr, limit, tov + 1 );
					}
				}
				return value;
			},
			 ...
			 */
			gatherClassName: function( elm, className, limit, tov ) {
				var value, tov = tov ? tov : 0,
					limit = limit ? limit : 3;
				...
				if ( _this.isElement( elm ) ) {
					...
					if ( !value && tov < limit ) {
						return _this.gatherClassName( elm.parentNode, className, limit, tov + 1 );
					} else if ( value ) {
						value = elm;
					}
				}
